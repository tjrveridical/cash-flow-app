...
import { formatPaymentRuleName } from "@/lib/types/forecast";
import { EditForecastItemModal } from "./EditForecastItemModal";

interface ForecastItemsTableProps {
  items: ForecastItemWithRule[];
  onDataChange: () => void;
}

export function ForecastItemsTable({ items, onDataChange }: ForecastItemsTableProps) {
  const [searchQuery, setSearchQuery] = useState("");
  const [editingItem, setEditingItem] = useState<ForecastItemWithRule | null>(null);
  ...
  async function handleDelete(itemId: string) {
    if (!confirm("Are you sure you want to delete this forecast item?")) {
      return;
    }

    try {
      const res = await fetch(`/api/forecast-items/${itemId}`, {
        method: "DELETE",
      });
      ...
      alert("Failed to delete forecast item");
    }
  }
            ...
            {filteredItems.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-3 py-8 text-center text-sm text-slate-500">
                  {searchQuery ? "No vendors match your search" : "No forecast items created yet"}
                </td>
              </tr>
            ) : (
                ...
                >
                  ...
                  <td className="px-3 py-2.5">
                    <span className="text-sm font-mono text-[#2d5a2d] font-medium">
                      {formatPaymentRuleName(item.rule)}
                    </span>
                  </td>
                  <td className="px-3 py-2.5">
